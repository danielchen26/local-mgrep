...
the cascade generalises beyond the warp Rust workspace.
...
The two honest misses (ANM "V6 biomarker resolve / chain", CCSB "auto-
...
## What's new
  ...
  - **`benchmarks/cross_repo/anm.json`** — 12 hand-labelled Python
    questions over the ANM scientific Python codebase, with
    `expected` paths and `ground_truth_note` for each.
  - **`benchmarks/cross_repo/ccsb.json`** — 12 hand-labelled
    TypeScript questions over the `claude-code-source-build` repo,
  ...
  - **`benchmarks/v0_7_multilang_bench.py`** — unified runner that
    ...
    cascade harness as the v0.5 warp bench, and reports both
...
## What did not change
  ...
  - The cascade default still hits 16 / 16 on warp.
...
## Empirical observations from the multi-language run
  ...
  - **The symbol layer (L2) and graph tiebreaker (L4) don't move
    ...
    the warp + ANM + CCSB benchmarks together still don't measure
    ...
    to discriminate.
  - **Cheap-path early-exit rate varies a lot by repo.** ANM's
    ...
    in warp.json rarely share surface tokens with the Rust crate
    names. CCSB sits between (4/12 cheap).
  - **CCSB indexer regression** noted: the L2 symbol extractor
    populated 0 symbols on CCSB and ANM (vs warp's 62 K symbols),
